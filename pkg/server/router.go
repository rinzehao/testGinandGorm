...
func BindRoute(orderHandler *OrderHandler) error {
	...
	r.GET("/order/query/:id", orderHandler.QueryOrderById)    //3)获取order的详情
	r.GET("/order/list", orderHandler.QueryAllOrders)         //4）获取order列表
	...
	r.DELETE("/order/del/:id", orderHandler.DeleteOrderById)  //8)删除order
