...
func BindRoute(orderHandler *handler.OrderHandler) {
	...
	r.GET("/order", orderHandler.GetOrders)                        //4）获取demo_order列表
	r.GET("/orderSearch", orderHandler.GetSortedOrders)            //5)获取模糊查询结果
	r.GET("/orderDownload", orderHandler.DownLoadExcel)            //6)下载xlsx表格
	r.POST("/orderUpload/:id", orderHandler.GetUploadUrlAndUpdate) //7)上传文件
	r.DELETE("/order/delete/:id", orderHandler.DeleteOrderById)    //8)删除demo_order
	r.Run()
}
